...
export class DatabaseService {
  ...
  static async queryScalar(text: string, params?: unknown[]): Promise<unknown> {
    const row = await this.queryOne(text, params);
    return row ? Object.values(row)[0] : null;
  }

  // Insert a record and return it
  static async insert<T = Record<string, unknown>>(table: string, data: Partial<T>): Promise<T> {
  ...
  static async updateOne<T = Record<string, unknown>>(
    table: string,
    where: string,
    whereParams: unknown[],
    data: Partial<T>
  ...
  // Delete records
  static async delete(table: string, where: string, params: unknown[]): Promise<number> {
...
export const isReservation = (value: Record<string, unknown>): value is Reservation => {
  return (
    typeof value.id === 'number' &&
    typeof value.carId === 'number' &&
    value.startDate instanceof Date &&
    value.endDate instanceof Date &&
    typeof value.status === 'string' &&
    ['PENDING', 'CONFIRMED', 'IN_PROGRESS', 'COMPLETED', 'RETURNED', 'CANCELLED'].includes(value.status as string) &&
