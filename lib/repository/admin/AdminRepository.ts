...
export async function getAdminReservations(
  filters: AdminReservationListFilters = {},
): Promise<AdminReservationListRow[]> {
  const conditions: string[] = [];
  const params: unknown[] = [];

  if (filters.companyId) {
    params.push(filters.companyId);
    conditions.push(`car."companyId" = $${params.length}`);
  }

  if (filters.dateFrom) {
    params.push(filters.dateFrom);
    conditions.push(`r."endDate" >= $${params.length}`);
  }

  if (filters.dateToExclusive) {
    params.push(filters.dateToExclusive);
    conditions.push(`r."startDate" < $${params.length}`);
  }
